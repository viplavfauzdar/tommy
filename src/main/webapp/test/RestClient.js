...
// Model for User entity
    models.User = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/webresources/com.financegeorgia.entities.user/",
        idAttribute: 'id',
        defaults: {
            lastName: "",
            password1: "",
    ...
    // Collection class for User entities
    models.UserCollection = Backbone.Collection.extend({
        model: models.User,
        url: "http://localhost:8080/webresources/com.financegeorgia.entities.user/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
// Model for com.financegeorgia.entities.User entity
    models.User1 = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/webresources/com.financegeorgia.entities.user/email/",
        idAttribute: 'id',
        defaults: {
            lastName: "",
            password1: "",
    ...
    // Collection class for com.financegeorgia.entities.User entities
    models.User1Collection = Backbone.Collection.extend({
        model: models.User1,
        url: "http://localhost:8080/webresources/com.financegeorgia.entities.user/",
        sync: function (method, model, options) {
            options || (options = {});
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        details: function (id) {
            if (this.collection) {
                ...
                this.view = new views.ModelView({
                    model: this.user,
                    // tpl-user-details is template identifier for chosen model element
                    templateName: '#tpl-user-details',
        ...
        getData: function () {
            return {
                ...
                email: $('#email').val(),
                pptosnda: $('#pptosnda').val(),
                locked: $('#locked').val(),
                firstName: $('#firstName').val(),
                mi: $('#mi').val(),
                aboutMe: $('#aboutMe').val(),
                passresetKey: $('#passresetKey').val(),
                userType: $('#userType').val()
            };
        }
    });
    new AppRouter();
    /*var AppRouter1 = Backbone.Router.extend({
     routes:{
     '':'list',
     ':id':'details'
     },
     ...
     list:function () {
     this.collection = new models.User1Collection();
     var self = this;
     this.collection.fetch({
     success:function () {
     self.listView = new views.ListView({
     model:self.collection,
     // tpl-user1-list-itemis template identifier for item
     templateName : '#tpl-user1-list-item'
     });
     $('#datatable').html(self.listView.render().el).append(_.template($('#thead').html())());
     if (self.requestedId) {
     self.details(self.requestedId);
     ...
     details:function (id){
     if (this.collection) {
     this.user1= this.collection.get(id);
     if (this.view) {
     this.view.close();
     }
     var self = this;
     this.view = new views.ModelView({
     model:this.user1,
     // tpl-user1-details is template identifier for chosen model element
     templateName: '#tpl-user1-details',
     getHashObject: function(){
     return self.getData();
     }
     });
     $('#details').html(this.view.render().el);} else {
     this.requestedId = id;
     this.list();
     }
     },
